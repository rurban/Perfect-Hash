...
# By default we try string keys. But if all keys are integers, e.g. unicode
# codepoints, use this.
my $int_keys = 0;
...
#
# The keys should be passed as an array reference.  They can be any
# set of Perl integers or strings; it is caller's responsibility that
# there not be any duplicates.  (Note that the "strings" can be binary
# data, but hashing e.g. OIDs has endianness hazards that callers must
# overcome.)
#
...
#
# int_keys => bool
# If specified as true, all keys are integers.
#
# case_fold => bool
...
sub generate_hash_function
{
	...
	# It's not worth passing these around as a parameter; just use globals.
	$int_keys = $options{int_keys} || 0;
  ...
  FIND_PARAMS:
	...
	if ($int_keys)
	{
		$f .= sprintf "%s(const int key)\n{\n", $funcname;
	}
	elsif (defined $options{fixed_key_length})
	{
	...
	$f .= sprintf "\tunsigned\t\ta = %d;\n",   $hash_seed1;
	$f .= sprintf "\tunsigned\t\tb = %d;\n\n", $hash_seed2;
        if ($int_keys) {
            $f .= sprintf "\ta = a * %d + key;\n", $hash_mult1;
            $f .= sprintf "\tb = b * %d + key;\n", $hash_mult2;
        } else {
